...
{{% blocks/products/pf/agp/content h2="How to Parse XHTML File Using Java" %}}
...
{{% blocks/products/pf/agp/feature-section-col title="Steps to Parse XHTML Files in Java" %}}
...
+  Load XHTML document using Document Class.
...
{{% blocks/products/pf/agp/code-block title="Parse XHTML Files - Java" offSpacer="" %}}

```cs
 //Load XHTML file containing images
Document xhtml = new Document(getMyDir() + "Images.xhtml");

// Get the collection of shapes.
NodeCollection shapes = xhtml.getChildNodes(NodeType.SHAPE, true);
